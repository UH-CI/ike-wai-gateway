...
AgaveToGo.config(function($translateProvider) {
  $translateProvider.translations('en', {
    ...
    creator: 'Authors of the data product.',
    ...
    subject: 'Keywords describing the resource. Consider search and retrieval in choices.',
    ...
    descriptor: 'A summary describing the resource.',
...
AgaveToGo.controller('IdleEventsCtrl', ['$scope', 'Idle', 'Keepalive','$uibModal', '$localStorage','$http','$window', function($scope, Idle, Keepalive,$uibModal, $localStorage, $http, $window) {
  ...
  $scope.isIdle =false;
    ...
    if($localStorage.token.expires_at >= expiring){
      $http.post('https://ikewai.its.hawaii.edu:8888/refresh?refresh_token='+$localStorage.token.refresh_token)
...
AgaveToGo.config(['$stateProvider', '$urlRouterProvider', '$urlMatcherFactoryProvider', function($stateProvider, $urlRouterProvider, $urlMatcherFactoryProvider) {
    ...
    $stateProvider
        ...
        .state("apps.rerun", {
          ...
          .state("parse-timeseries", {
            url: "/parse-timeseries",
            controller: "ParseTimeseriesController",
            templateUrl: "views/apps/resource/parse-timeseries.html",
            resolve: {
                deps: ['$ocLazyLoad', function($ocLazyLoad) {
                  return $ocLazyLoad.load([
                    {
